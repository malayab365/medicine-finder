...
Endpoints: `POST /search/name` (`{query}` → label JSON + `adverse_events`) and `POST /search/symptom` (`{symptoms}` → candidate list + each candidate's OpenFDA label), both in `app/medicines/router.py` and delegating to `app/medicines/service.py`. Both are rate-limited per client IP (`app/shared/ratelimit.py`; symptom is stricter since it calls the paid LLM). **Symptom search additionally requires a logged-in user** (`Depends(auth.deps.require_user)` → 401 if anonymous); name search stays public.
